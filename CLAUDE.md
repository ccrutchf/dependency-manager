...
**Root requirement** — `RootCheck.PlanRequiresRoot` returns true if the plan has any PPAs or apt/snap packages. `InstallCommand` refuses to run in that case without euid 0 (checked via `libc.geteuid` P/Invoke). Flatpak falls back to `--user` scope when not root. `plan` and `test` never need root.

## CLI entry

`Program.cs` uses `System.CommandLine` 2.0 beta 4. Subcommands: `plan`, `install` (supports `--fail-fast`), `test`, `list`. All package-aware commands accept `--config`/`-c` (default `packages.yaml`). Commands live in `Commands/` as static classes with `Run`/`RunAsync`.
